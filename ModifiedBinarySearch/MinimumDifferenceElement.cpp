...
If we can’t find the ‘key’, (at the end of the loop) we can find the differences between the ‘key’ and the numbers pointed out by 
indices start and end, as these two numbers will be closest to the ‘key’. 
...
public:
    static int searchMinDiffElement(const vector<int>& arr, int key) {
        ...
        return (abs(key - arr[start]) < abs(key - arr[end])) ? arr[start] : arr[end];
    }
};
