...
private:
    static int findPivot(const vector<int>& arr) {
    ...
    static int binarySearch1(const vector<int>& arr, int key, int start, int end) {
        while (start <= end) {
            int mid = start + (end - start) / 2;
    ...
    static int binarySearch2(const vector<int>& arr, int key, int start, int end) {
        while (start <= end) {
            int mid = start + (end - start) / 2;
            if (key == arr[mid]) return mid;
...
public:
    static int search(const vector<int>& arr, int key) {
        ...
        int result = binarySearch1(arr, key, 0, pivot);
        if (result == -1)
            result = binarySearch2(arr, key, pivot, arr.size() - 1);
        return result;
    }
};
