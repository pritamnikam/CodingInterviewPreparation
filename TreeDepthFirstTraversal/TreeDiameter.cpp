...
the height of the right child plus ‘1’ for the current node: diameter = leftTreeHeight + rightTreeHeight + 1. 

To find the overall tree diameter, we will use a class level variable. This variable will store the maximum diameter 
of all the nodes visited so far, hence, eventually, it will have the final tree diameter.
...
public:
    ...
    static int calculateHeight(TreeNode* currentNode, int& treeDiameter) {
        ...
        int leftHeight = calculateHeight(currentNode->left, treeDiameter);
        int rightHeight = calculateHeight(currentNode->right, treeDiameter);
