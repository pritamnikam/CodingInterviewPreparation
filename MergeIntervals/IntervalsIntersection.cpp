...
public:
    static vector<Interval> merge(const vector<Interval>& arr1, const vector<Interval>& arr2) {
        ...
        while (i < arr1.size() && j < arr2.size()) {
            // if intervals in list are not overlapping move forward
            if (arr1[i].end < arr2[j].start) ++i;

            // if intervals in list are not overlapping move forward
            else if (arr1[i].start > arr2[j].end) ++j;

            else {
                // get only overlapping section of the intervals
                Interval it(max(arr1[i].start, arr2[j].start),
                    min(arr1[i].end, arr2[j].end));

                result.push_back(it);

                // if intervals in list are overlapping move forward
...
So our algorithm will be to iterate through both the lists together to see if any two intervals overlap.If two intervals overlap, we will insert the overlapped part into a result listand move on to the next interval which is finishing early.
